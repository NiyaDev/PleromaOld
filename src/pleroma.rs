...
impl Default for Pleroma {
	fn default() -> Self {
		...
		Self {
			...
			screen_size: Resolution { width: 1280, height: 720 },
			render_size: Resolution { width:  640, height: 360 },
			framerate: 60,
...
impl Pleroma {
	...
	pub fn set_render(&mut self, width: i32, height: i32) -> &mut Self {
		if !self.is_rendering {
			...
			self.update_render();
		} else { self.log(PlError::EditingRenderSize) }
	...
	pub fn fullscreen(&mut self) -> &mut Self {
		...
		if is_full { self.windows_flags.bitxor_assign(WindowFlags::FULLSCREEN) }
	...
	pub fn borderless(&mut self) -> &mut Self {
		...
		if is_full { self.windows_flags.bitxor_assign(WindowFlags::BORDERLESS) }
